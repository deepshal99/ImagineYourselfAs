...
PROMPT_FAST = """Create a high-quality movie poster featuring the person in the uploaded image as a main character in the Fast & Furious universe.
...
# Forcing the update by including a change in reference_description as well if prompt alone isn't triggering?
# No, let's just try PATCH one more time with simple JSON.

for p_id, prompt in [("movie_oppenheimer", PROMPT_OPPEN), ("movie_fast", PROMPT_FAST)]:
    print(f"Updating {p_id}...")
    r = requests.patch(f"{REST_URL}?id=eq.{p_id}", headers=headers, json={"prompt": prompt})
