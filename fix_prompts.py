...
NEW_PROMPTS = {
    "movie_oppenheimer": """Create a high-quality movie poster featuring the person in the uploaded image as a main character in the Oppenheimer universe.
    ...
    "movie_fast": """Create a high-quality movie poster featuring the person in the uploaded image as a main character in the Fast & Furious universe.
...
def update_persona(persona_id, prompt):
    print(f"  Updating: {persona_id}...")
    resp = requests.patch(
        f"{REST_URL}?id=eq.{persona_id}", 
        headers=headers, 
        json={"prompt": prompt}
    )
...
def main():
    for p_id, prompt in NEW_PROMPTS.items():
        update_persona(p_id, prompt)
