...
serve(async (req) => {
  ...
  try {
    ...
    if (!faceDescription) {
      const visionResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
        method: 'POST',
    ...
    // Add timeout to prevent Edge Function timeout (max 150s, so use 120s)
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        throw new Error("Generation timed out. The AI model is taking too long. Please try again.");
      }
      throw fetchError;
    } finally {
